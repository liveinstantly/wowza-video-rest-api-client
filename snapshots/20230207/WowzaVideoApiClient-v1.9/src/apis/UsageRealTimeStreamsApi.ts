...
import {
    ...
    Error422FromJSON,
    Error422ToJSON,
    UsageRealTimeStreamFromJSON,
    UsageRealTimeStreamToJSON,
    UsageRealTimeStreamsFromJSON,
    UsageRealTimeStreamsToJSON,
} from '../models';

export interface ShowUsageRealTimeStreamRequest {
    id: string;
    from?: Date;
    to?: Date;
}

export interface ShowUsageRealTimeStreamsIndexRequest {
    from?: Date;
    to?: Date;
    nextPageKey?: string;
    perPage?: number;
 ...
 * 
 */
export class UsageRealTimeStreamsApi extends runtime.BaseAPI {
     ...
     */
    async showUsageRealTimeStreamRaw(requestParameters: ShowUsageRealTimeStreamRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<UsageRealTimeStream>> {
        if (requestParameters.id === null || requestParameters.id === undefined) {
            throw new runtime.RequiredError('id','Required parameter requestParameters.id was null or undefined when calling showUsageRealTimeStream.');
        }
        ...
        const response = await this.request({
            path: `/usage/real_time_streams/{id}`.replace(`{${"id"}}`, encodeURIComponent(String(requestParameters.id))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        }, initOverrides);

        return new runtime.JSONApiResponse(response, (jsonValue) => UsageRealTimeStreamFromJSON(jsonValue));
    }
     ...
     * Fetch usage for a single real time stream
     */
    async showUsageRealTimeStream(requestParameters: ShowUsageRealTimeStreamRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<UsageRealTimeStream> {
        const response = await this.showUsageRealTimeStreamRaw(requestParameters, initOverrides);
     ...
     */
    async showUsageRealTimeStreamsIndexRaw(requestParameters: ShowUsageRealTimeStreamsIndexRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<UsageRealTimeStreams>> {
        const queryParameters: any = {};
        ...
        return new runtime.JSONApiResponse(response, (jsonValue) => UsageRealTimeStreamsFromJSON(jsonValue));
    }
     ...
     * Fetch usage for all real time streams
     */
    async showUsageRealTimeStreamsIndex(requestParameters: ShowUsageRealTimeStreamsIndexRequest = {}, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<UsageRealTimeStreams> {
        const response = await this.showUsageRealTimeStreamsIndexRaw(requestParameters, initOverrides);
        return await response.value();
    }
