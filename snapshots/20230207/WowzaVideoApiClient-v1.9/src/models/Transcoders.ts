...
import type { IndexTranscoder } from './IndexTranscoder';
import {
    IndexTranscoderFromJSON,
    IndexTranscoderFromJSONTyped,
    IndexTranscoderToJSON,
} from './IndexTranscoder';
...
export interface Transcoders {
    /**
     * 
     * @type {Array<IndexTranscoder>}
     * @memberof Transcoders
     */
    transcoders: Array<IndexTranscoder>;
}

/**
 * Check if a given object implements the Transcoders interface.
 */
export function instanceOfTranscoders(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "transcoders" in value;

    return isInstance;
}

export function TranscodersFromJSON(json: any): Transcoders {
    return TranscodersFromJSONTyped(json, false);
}

export function TranscodersFromJSONTyped(json: any, ignoreDiscriminator: boolean): Transcoders {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'transcoders': ((json['transcoders'] as Array<any>).map(IndexTranscoderFromJSON)),
    };
}

export function TranscodersToJSON(value?: Transcoders | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'transcoders': ((value.transcoders as Array<any>).map(IndexTranscoderToJSON)),
    };
}
