...
/**
 * Check if a given object implements the PlayerUrl interface.
 */
export function instanceOfPlayerUrl(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function PlayerUrlFromJSON(json: any): PlayerUrl {
    return PlayerUrlFromJSONTyped(json, false);
}

export function PlayerUrlFromJSONTyped(json: any, ignoreDiscriminator: boolean): PlayerUrl {
...
export function PlayerUrlToJSON(value?: PlayerUrl | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
