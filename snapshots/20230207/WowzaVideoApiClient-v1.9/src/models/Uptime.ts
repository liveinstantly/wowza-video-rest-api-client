...
/**
 * Check if a given object implements the Uptime interface.
 */
export function instanceOfUptime(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function UptimeFromJSON(json: any): Uptime {
    return UptimeFromJSONTyped(json, false);
}

export function UptimeFromJSONTyped(json: any, ignoreDiscriminator: boolean): Uptime {
...
export function UptimeToJSON(value?: Uptime | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
