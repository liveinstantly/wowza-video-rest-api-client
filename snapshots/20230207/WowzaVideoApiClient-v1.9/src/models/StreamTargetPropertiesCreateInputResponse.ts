...
import type { StreamTargetProperty } from './StreamTargetProperty';
import {
    StreamTargetPropertyFromJSON,
    StreamTargetPropertyFromJSONTyped,
    StreamTargetPropertyToJSON,
} from './StreamTargetProperty';

/**
 * 
 * @export
 * @interface StreamTargetPropertiesCreateInputResponse
 */
export interface StreamTargetPropertiesCreateInputResponse {
    /**
     * An array of properties. The property configuration consists of a key/value pair and the section of the stream target configuration table the key/value pair is stored in. You can only set an array of properties on a Fastly or Akamai stream target. Available from version 1.7.
     * @type {Array<StreamTargetProperty>}
     * @memberof StreamTargetPropertiesCreateInputResponse
     */
    properties: Array<StreamTargetProperty>;
}

/**
 * Check if a given object implements the StreamTargetPropertiesCreateInputResponse interface.
 */
export function instanceOfStreamTargetPropertiesCreateInputResponse(value: object): boolean {
    let isInstance = true;
...
export function StreamTargetPropertiesCreateInputResponseFromJSON(json: any): StreamTargetPropertiesCreateInputResponse {
    return StreamTargetPropertiesCreateInputResponseFromJSONTyped(json, false);
}

export function StreamTargetPropertiesCreateInputResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): StreamTargetPropertiesCreateInputResponse {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'properties': ((json['properties'] as Array<any>).map(StreamTargetPropertyFromJSON)),
    };
}

export function StreamTargetPropertiesCreateInputResponseToJSON(value?: StreamTargetPropertiesCreateInputResponse | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'properties': ((value.properties as Array<any>).map(StreamTargetPropertyToJSON)),
    };
}
