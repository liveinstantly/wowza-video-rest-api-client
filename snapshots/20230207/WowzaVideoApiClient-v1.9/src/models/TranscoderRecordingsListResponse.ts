...
import type { IndexRecordings } from './IndexRecordings';
import {
    IndexRecordingsFromJSON,
    IndexRecordingsFromJSONTyped,
    IndexRecordingsToJSON,
} from './IndexRecordings';

/**
 * 
 * @export
 * @interface TranscoderRecordingsListResponse
 */
export interface TranscoderRecordingsListResponse {
    /**
     * Array of recordings of this transcoder. See /recordings for more details.
     * @type {Array<IndexRecordings>}
     * @memberof TranscoderRecordingsListResponse
     */
    recordings: Array<IndexRecordings>;
}

/**
 * Check if a given object implements the TranscoderRecordingsListResponse interface.
 */
export function instanceOfTranscoderRecordingsListResponse(value: object): boolean {
    let isInstance = true;
...
export function TranscoderRecordingsListResponseFromJSON(json: any): TranscoderRecordingsListResponse {
    return TranscoderRecordingsListResponseFromJSONTyped(json, false);
}

export function TranscoderRecordingsListResponseFromJSONTyped(json: any, ignoreDiscriminator: boolean): TranscoderRecordingsListResponse {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'recordings': ((json['recordings'] as Array<any>).map(IndexRecordingsFromJSON)),
    };
}

export function TranscoderRecordingsListResponseToJSON(value?: TranscoderRecordingsListResponse | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'recordings': ((value.recordings as Array<any>).map(IndexRecordingsToJSON)),
    };
}
