...
import type { IndexPlayer } from './IndexPlayer';
import {
    IndexPlayerFromJSON,
    IndexPlayerFromJSONTyped,
    IndexPlayerToJSON,
} from './IndexPlayer';
...
/**
 * Check if a given object implements the Players interface.
 */
export function instanceOfPlayers(value: object): boolean {
    let isInstance = true;
...
export function PlayersFromJSON(json: any): Players {
    return PlayersFromJSONTyped(json, false);
}

export function PlayersFromJSONTyped(json: any, ignoreDiscriminator: boolean): Players {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'players': ((json['players'] as Array<any>).map(IndexPlayerFromJSON)),
    };
}

export function PlayersToJSON(value?: Players | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'players': ((value.players as Array<any>).map(IndexPlayerToJSON)),
    };
}
