...
/**
 * Check if a given object implements the IndexPlayer interface.
 */
export function instanceOfIndexPlayer(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function IndexPlayerFromJSON(json: any): IndexPlayer {
    return IndexPlayerFromJSONTyped(json, false);
}

export function IndexPlayerFromJSONTyped(json: any, ignoreDiscriminator: boolean): IndexPlayer {
...
export function IndexPlayerToJSON(value?: IndexPlayer | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
