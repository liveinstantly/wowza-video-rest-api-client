...
/**
 * Check if a given object implements the HeightMetric interface.
 */
export function instanceOfHeightMetric(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function HeightMetricFromJSON(json: any): HeightMetric {
    return HeightMetricFromJSONTyped(json, false);
}

export function HeightMetricFromJSONTyped(json: any, ignoreDiscriminator: boolean): HeightMetric {
...
export function HeightMetricToJSON(value?: HeightMetric | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
