...
import type { IndexVodStream } from './IndexVodStream';
import {
    IndexVodStreamFromJSON,
    IndexVodStreamFromJSONTyped,
    IndexVodStreamToJSON,
} from './IndexVodStream';

/**
 * 
 * @export
 * @interface VodStreams
 */
export interface VodStreams {
    /**
     * 
     * @type {Array<IndexVodStream>}
     * @memberof VodStreams
     */
    vodStreams: Array<IndexVodStream>;
}

/**
 * Check if a given object implements the VodStreams interface.
 */
export function instanceOfVodStreams(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "vodStreams" in value;

    return isInstance;
}

export function VodStreamsFromJSON(json: any): VodStreams {
    return VodStreamsFromJSONTyped(json, false);
}

export function VodStreamsFromJSONTyped(json: any, ignoreDiscriminator: boolean): VodStreams {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'vodStreams': ((json['vod_streams'] as Array<any>).map(IndexVodStreamFromJSON)),
    };
}

export function VodStreamsToJSON(value?: VodStreams | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'vod_streams': ((value.vodStreams as Array<any>).map(IndexVodStreamToJSON)),
    };
}
