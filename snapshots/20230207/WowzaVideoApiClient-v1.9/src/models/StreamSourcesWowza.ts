...
import type { IndexStreamSourceWowza } from './IndexStreamSourceWowza';
import {
    IndexStreamSourceWowzaFromJSON,
    IndexStreamSourceWowzaFromJSONTyped,
    IndexStreamSourceWowzaToJSON,
} from './IndexStreamSourceWowza';

/**
 * 
 * @export
 * @interface StreamSourcesWowza
 */
export interface StreamSourcesWowza {
    /**
     * 
     * @type {Array<IndexStreamSourceWowza>}
     * @memberof StreamSourcesWowza
     */
    streamSourcesWowza: Array<IndexStreamSourceWowza>;
}

/**
 * Check if a given object implements the StreamSourcesWowza interface.
 */
export function instanceOfStreamSourcesWowza(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "streamSourcesWowza" in value;

    return isInstance;
}

export function StreamSourcesWowzaFromJSON(json: any): StreamSourcesWowza {
    return StreamSourcesWowzaFromJSONTyped(json, false);
}

export function StreamSourcesWowzaFromJSONTyped(json: any, ignoreDiscriminator: boolean): StreamSourcesWowza {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'streamSourcesWowza': ((json['stream_sources_wowza'] as Array<any>).map(IndexStreamSourceWowzaFromJSON)),
    };
}

export function StreamSourcesWowzaToJSON(value?: StreamSourcesWowza | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'stream_sources_wowza': ((value.streamSourcesWowza as Array<any>).map(IndexStreamSourceWowzaToJSON)),
    };
}
