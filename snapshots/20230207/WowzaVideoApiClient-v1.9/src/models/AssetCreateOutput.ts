...
/**
 * Check if a given object implements the AssetCreateOutput interface.
 */
export function instanceOfAssetCreateOutput(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function AssetCreateOutputFromJSON(json: any): AssetCreateOutput {
    return AssetCreateOutputFromJSONTyped(json, false);
}

export function AssetCreateOutputFromJSONTyped(json: any, ignoreDiscriminator: boolean): AssetCreateOutput {
...
export function AssetCreateOutputToJSON(value?: AssetCreateOutput | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
