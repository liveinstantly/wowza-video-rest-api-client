...
import type { UsageTranscoderSummaryDataPerConfiguration } from './UsageTranscoderSummaryDataPerConfiguration';
import {
    UsageTranscoderSummaryDataPerConfigurationFromJSON,
    UsageTranscoderSummaryDataPerConfigurationFromJSONTyped,
    UsageTranscoderSummaryDataPerConfigurationToJSON,
} from './UsageTranscoderSummaryDataPerConfiguration';

/**
 * 
 * @export
 * @interface UsageTranscoderSummaryData
 */
export interface UsageTranscoderSummaryData {
    /**
     * The total amount of content, in bytes, that went through all transcoders during the selected time frame.
     * @type {number}
     * @memberof UsageTranscoderSummaryData
     */
    egressBytes?: number;
    /**
     * The total amount of time, in seconds, that it took for all transcoders to be processed.
     * @type {number}
     * @memberof UsageTranscoderSummaryData
     */
    egressSeconds?: number;
    /**
     * An array of transcoder summary data by transcoder configuration.
     * @type {Array<UsageTranscoderSummaryDataPerConfiguration>}
     * @memberof UsageTranscoderSummaryData
     */
    configurations?: Array<UsageTranscoderSummaryDataPerConfiguration>;
}

/**
 * Check if a given object implements the UsageTranscoderSummaryData interface.
 */
export function instanceOfUsageTranscoderSummaryData(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function UsageTranscoderSummaryDataFromJSON(json: any): UsageTranscoderSummaryData {
    return UsageTranscoderSummaryDataFromJSONTyped(json, false);
}

export function UsageTranscoderSummaryDataFromJSONTyped(json: any, ignoreDiscriminator: boolean): UsageTranscoderSummaryData {
    ...
    return {
        ...
        'configurations': !exists(json, 'configurations') ? undefined : ((json['configurations'] as Array<any>).map(UsageTranscoderSummaryDataPerConfigurationFromJSON)),
    };
}

export function UsageTranscoderSummaryDataToJSON(value?: UsageTranscoderSummaryData | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'configurations': value.configurations === undefined ? undefined : ((value.configurations as Array<any>).map(UsageTranscoderSummaryDataPerConfigurationToJSON)),
    };
}
