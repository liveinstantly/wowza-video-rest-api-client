...
import type { DirectPlaybackUrl } from './DirectPlaybackUrl';
import {
    DirectPlaybackUrlFromJSON,
    DirectPlaybackUrlFromJSONTyped,
    DirectPlaybackUrlToJSON,
} from './DirectPlaybackUrl';
 ...
 * @export
 * @interface DirectPlaybackUrls
 */
export interface DirectPlaybackUrls {
    /**
     * The name of the direct playback protocol.
     * @type {Array<DirectPlaybackUrl>}
     * @memberof DirectPlaybackUrls
     */
    deliveryProtocol?: Array<DirectPlaybackUrl>;
}

/**
 * Check if a given object implements the DirectPlaybackUrls interface.
 */
export function instanceOfDirectPlaybackUrls(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function DirectPlaybackUrlsFromJSON(json: any): DirectPlaybackUrls {
    return DirectPlaybackUrlsFromJSONTyped(json, false);
}

export function DirectPlaybackUrlsFromJSONTyped(json: any, ignoreDiscriminator: boolean): DirectPlaybackUrls {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'deliveryProtocol': !exists(json, 'delivery_protocol') ? undefined : ((json['delivery_protocol'] as Array<any>).map(DirectPlaybackUrlFromJSON)),
    };
}

export function DirectPlaybackUrlsToJSON(value?: DirectPlaybackUrls | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'delivery_protocol': value.deliveryProtocol === undefined ? undefined : ((value.deliveryProtocol as Array<any>).map(DirectPlaybackUrlToJSON)),
    };
}
