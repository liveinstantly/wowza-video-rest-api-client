...
/**
 * Check if a given object implements the Uptimes interface.
 */
export function instanceOfUptimes(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "uptimes" in value;

    return isInstance;
}

export function UptimesFromJSON(json: any): Uptimes {
    return UptimesFromJSONTyped(json, false);
}

export function UptimesFromJSONTyped(json: any, ignoreDiscriminator: boolean): Uptimes {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'uptimes': ((json['uptimes'] as Array<any>).map(UptimeFromJSON)),
    };
}

export function UptimesToJSON(value?: Uptimes | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'uptimes': ((value.uptimes as Array<any>).map(UptimeToJSON)),
    };
}
