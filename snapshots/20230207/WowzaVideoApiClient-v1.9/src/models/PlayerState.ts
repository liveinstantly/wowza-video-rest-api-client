 ...
 * @export
 */
export const PlayerStateStateEnum = {
    Requested: 'requested',
    Activated: 'activated'
} as const;
export type PlayerStateStateEnum = typeof PlayerStateStateEnum[keyof typeof PlayerStateStateEnum];


/**
 * Check if a given object implements the PlayerState interface.
 */
export function instanceOfPlayerState(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function PlayerStateFromJSON(json: any): PlayerState {
    return PlayerStateFromJSONTyped(json, false);
}

export function PlayerStateFromJSONTyped(json: any, ignoreDiscriminator: boolean): PlayerState {
...
export function PlayerStateToJSON(value?: PlayerState | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
